...
2. **burger_full**
   Создает полностью собранный бургер, данные берутся из `data.py`:
...
4. **mocked_burger_full**
   Создает полностью собранный бургер с мокированными атрибутами (данные берутся из `data.py`):
   - Мок булочки
   - Мок одного ингредиента